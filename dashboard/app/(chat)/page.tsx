...
import { Loader2, Snowflake, Droplets, Wind, CloudRain, CloudSnow, Sun, Cloud, CloudSun, Gauge, TrendingUp, TrendingDown, Minus, Settings, LogOut, User, X } from "lucide-react";
import { createClient } from "@/lib/supabase/client";
// Theme is driven by weather + time of day — no manual light/dark toggle
...
const GLASS_STYLES: Record<TimeOfDay, { card: string; cardBorder: string; sheet: string }> = {
  ...
  noon:      { card: "bg-blue-700/40",     cardBorder: "border-blue-200/15",     sheet: "bg-blue-950/90" },
      ...
      <div className="relative w-36 h-18.75">
        <svg viewBox="0 0 140 75" className="w-full h-full">
          ...
          <motion.path d="M 10 68 A 60 60 0 0 1 130 68" fill="none" stroke="currentColor" strokeWidth="10" strokeLinecap="round"
            className={meta.color || "text-white/30"}
  ...
  return (
    <div className="w-full">
      <svg viewBox="0 0 300 60" className="w-full h-auto" preserveAspectRatio="none">
        ...
        <polygon points={fillPoints} fill="url(#dashSparkFill)" />
...
function ChatPageInner() {
  ...
  // No manual theme toggle — theme is driven by weather + time of day
  ...
  useEffect(() => { if (user) fetchChats(); }, [user, fetchChats]);
  ...
  const fetchDashboardPrediction = useCallback(async (day: string, hour: number) => {
    ...
    try {
      ...
      updateFromPrediction(await res.json(), selectedModel);
    } catch { setDashError("Failed to connect to prediction service"); }
    finally { setDashLoading(false); }
  }, [selectedModel, updateFromPrediction]);

  // Auto-predict on load
  useEffect(() => {
    if (user && !initialFetched.current) {
      initialFetched.current = true;
      fetchDashboardPrediction(utahNow.dayOfWeek, utahNow.hour);
    }
  }, [user]);

  const handleDayChange = (day: string, label: string) => {
    setSelectedDay(day); setSelectedDayLabel(label);
    fetchDashboardPrediction(day, selectedHour);
  };
  const handleHourChange = (hour: number) => {
    setSelectedHour(hour);
    fetchDashboardPrediction(selectedDay, hour);
  };
  /* ---- Chat ---- */
  const handleSendMessage = async (content: string) => {
    ...
    const body: Record<string, unknown> = { chatId: currentChatId, content, model: selectedModel };
    ...
    try {
      ...
      let fullContent = ""; let newChatId: string | null = null;
      let newTitle: string | null = null; let responseMeta: MessageMeta | undefined;
      ...
      while (true) {
        ...
        for (const line of decoder.decode(value).split("\n")) {
          ...
          try {
            ...
            if (parsed.chatId) { newChatId = parsed.chatId; setCurrentChatId(newChatId); }
      ...
      if (responseMeta?.debug?.mlResponse) {
        updateFromPrediction(responseMeta.debug.mlResponse as Record<string, unknown>, responseMeta.model,
          responseMeta.debug.mlRequest as Record<string, unknown> | undefined);
      }
      if (newChatId || newTitle) fetchChats();
    } catch (error) {
      if ((error as Error).name !== "AbortError") {
        setMessages((prev) => prev.filter((m) => m.id !== userMessage.id));
      }
    } finally { setIsLoading(false); setStreamingContent(""); abortControllerRef.current = null; }
  };

  const handleStop = () => abortControllerRef.current?.abort();
  const handleSelectChat = (chatId: string | null) => { setCurrentChatId(chatId); if (!chatId) setMessages([]); };
            ...
            <div className="flex items-center gap-1.5">
              ...
              <div className="relative" ref={menuRef}>
                <button onClick={() => setMenuOpen(!menuOpen)}
                  className={cn("rounded-full p-1.5 transition-colors border border-white/10",
                    menuOpen ? "bg-white/20 text-white" : "bg-white/10 text-white/50 hover:bg-white/15")}
                  title="Menu"
                >
                ...
                <AnimatePresence>
                  {menuOpen && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-48 rounded-xl overflow-hidden z-50 shadow-[0_8px_40px_rgba(0,0,0,0.4)]"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-5">
            ...
            <AnimatePresence mode="wait">
              <motion.div key={condition + String(prediction)}
                ...
                className={cn("rounded-2xl backdrop-blur-lg p-4 sm:p-5 flex flex-col justify-between", glass.card, `border ${glass.cardBorder}`)}
              >
                {hasWeather ? (
                  <div className="flex items-center gap-4">
                    <condMeta.Icon className="h-16 w-16 sm:h-20 sm:w-20 lg:h-24 lg:w-24 text-white/80 shrink-0" strokeWidth={1} />
                    <div>
                      <div className="flex items-end gap-1">
                        <span className="text-5xl sm:text-7xl lg:text-8xl font-bold text-white leading-none tracking-tight">{Math.round(temp_f!)}°</span>
                        <span className="text-xl sm:text-2xl text-white/50 mb-2 sm:mb-3">F</span>
                      </div>
                      <p className="text-base sm:text-lg text-white/70 font-medium mt-1">{condMeta.label}</p>
                    </div>
                  </div>
                ) : dashLoading ? (
                  <div className="flex items-center gap-4">
                    <Loader2 className="h-16 w-16 sm:h-20 sm:w-20 text-white/30 animate-spin shrink-0" />
                    <div>
                      <span className="text-4xl sm:text-5xl font-bold text-white/30">--°</span>
                      <p className="text-sm text-white/60 mt-1">Fetching prediction...</p>
                    </div>
                  </div>
                ) : (
                  <div className="flex items-center gap-4">
                    <Gauge className="h-14 w-14 sm:h-20 sm:w-20 text-white/60 shrink-0" strokeWidth={1} />
                    <div>
                      <p className="text-lg sm:text-xl text-white/60">Weather data unavailable</p>
                      <p className="text-xs sm:text-sm text-white/60 mt-1">No sensor data for this period</p>
                    </div>
                  </div>
                )}
            ...
            <motion.div
              ...
              className={cn("rounded-2xl backdrop-blur-lg p-4 sm:p-5 flex flex-col", glass.card, `border ${glass.cardBorder}`)}
            >
      ...
      >
        {messages.length > 0 || isLoading ? (
          <ChatMessages messages={messages} isLoading={isLoading} streamingContent={streamingContent}
            ...
            onResendMessage={(c) => handleSendMessage(c)}
